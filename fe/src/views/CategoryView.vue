            ...
            <tbody>
              ...
              >
                ...
                <td class="px-6 py-4">
                  <button
                    type="button"
                    @click="editCategory(item.id, $event)"
...
export default {
  ...
  methods: {
    cek() {
      ...
      axios
        .get(
          `/api/v1/sort/category/${this.sort}/${this.selectedSortBy}?search=${search}`
        )
        .then((response) => {
    ...
    editCategory(id, event) {
      ...
      localStorage.setItem("history", this.$route.name);
      this.$store.state.history = this.$route.name;
      this.$router.push("category/editcategory/" + id);
    },
    navigate() {
      localStorage.setItem("history", this.$route.name);
      this.$store.state.history = this.$route.name;
