          ...
          >
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-6 py-4 capitalize">{{ item.color }}</td>
                <td class="px-6 py-4">
                  <button
                    type="button"
                    @click="editProduct(item.id, $event)"
...
export default {
  ...
  data() {
    return {
      phone: [],
      sortBy: [{ name: "Color", current: true }],
      sortOptions: [
  ...
  methods: {
    cek() {
      ...
      this.phone = [];
      axios
        .get(`/api/v1/sort/color/${this.sort}?search=${search}`)
        .then((response) => {
    ...
    editProduct(id, event) {
      ...
      localStorage.setItem("history", this.$route.name);
      this.$store.state.history = this.$route.name;
      this.$router.push("color/editcolor/" + id);
    },
    navigate() {
      localStorage.setItem("history", this.$route.name);
      this.$store.state.history = this.$route.name;
      this.$router.push("/color/addcolor");
    },
    fetchData() {
      this.loading = true;
      axios
