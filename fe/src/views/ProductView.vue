            ...
            <tbody>
              ...
              >
                ...
                <td class="px-6 py-4 capitalize">
                  {{ item.product.sub_category.category.category }}
                </td>
                <td class="px-6 py-4 capitalize">
                  {{ item.product.sub_category.sub_category }}
                </td>
                <td class="px-6 py-4">
                  Rp{{
                    item.product.price
                ...
                <td class="px-6 py-4">
                  <button
                    type="button"
                    @click="editProduct(item.product_id, $event)"
                  ...
                  <button
                    @click="openModals(item.product_id, $event)"
                    type="button"
...
export default {
  ...
  data() {
    return {
      ...
      sortBy: [
        ...
        { name: "Sub Category", current: false },
  ...
  methods: {
    cek() {
      ...
      axios
        .get(
          `/api/v1/sort/${this.sort}/${this.selectedSortBy}?search=${search}`
        )
        .then((response) => {
    ...
    deleteProduct() {
      axios
        .delete(`/api/v1/product/${this.product_id}`)
        .then((response) => {
          console.log(response.data);
          this.open = false;
          this.fetchData();
          // Handle respons sukses
          // Misalnya, tampilkan pesan sukses atau refresh tampilan produk
        })
        .catch((error) => {
          console.error(error);
    ...
    editProduct(id, event) {
      ...
      localStorage.setItem("history", this.$route.name);
      this.$store.state.history = this.$route.name;
      this.$router.push("product/editproduct/" + id);
    },
